...
//    parse: A function to modify the incoming data. (Doesn't do anything by default)
//
...
(function($) {
  ...
  <div id="pageless_loader" style="display:none;text-align:center;width:100%;"> \
	...
	parse: function(data) { return data; }	// Don't do anything by default
  };
  ...
  $.pageless.scroll = function () {
    ...
    if(!$.pageless.is_loading && (distance_to_end < $.pageless.settings.distance)) {
      ...
      // finally ajax query
      $.ajax({dataType:'script', type:'get', url:$.pageless.settings.url}, function(data){
				var data = $.pageless.settings.parse(data);				
        $.pageless.loader ?
